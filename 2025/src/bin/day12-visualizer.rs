...
use advent_of_code_2025::day12::{parse, place, try_place, unplace, Input};
...
async fn render_loop(rx: Receiver<Event>) {
    ...
    loop {
        ...
        draw_text(&region_text, GRID_AREA - 10.0 - region_width, 48.0, 20.0, DARKGRAY);
...
fn draw_sidebar(shape_previews: &[Vec<(i32, i32)>], counts: &[usize]) {
    ...
    draw_line(SIDEBAR_X, y_cursor, SIDEBAR_X + SIDEBAR_W - 10.0, y_cursor, 1.0, DARKGRAY);
    y_cursor += 15.0;
    ...
    for (type_idx, cells) in shape_previews.iter().enumerate() {
        ...
        for &(r, c) in cells {
            ...
            draw_rectangle(x + 1.0, y + 1.0, preview_cell - 2.0, preview_cell - 2.0, dimmed);
        }
