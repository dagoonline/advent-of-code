...
fn is_valid_part2(n: u64) -> bool {
    ...
    while chunk_size <= s.len() / 2 {
        let sub = &s[0..chunk_size];
        let mut from = chunk_size;
        let mut is_valid = false;

        while !is_valid && from < s.len() {
...
fn solver(input: &Vec<(u64, u64)>, is_valid: &dyn Fn(u64) -> bool) -> u64 {
    let mut counter = 0;

    for (start, end) in input {
        counter += (*start..*end + 1).filter(|v| !is_valid(*v)).sum::<u64>();
    }
